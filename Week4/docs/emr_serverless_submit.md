# EMR Serverless Submit (Gold Job)

This runbook submits `Week4/code/spark_to_gold.py` to EMR Serverless.
...
$required = @('AWS_PROFILE','AWS_REGION','SILVER_URI','GOLD_URI','SPARK_ADAPTIVE_ENABLED','SPARK_SHUFFLE_PARTITIONS','SPARK_TARGET_FILES_PER_PARTITION')
...
if ($SILVER_URI_S3 -notmatch '^s3://([^/]+)/') {
  throw "SILVER_URI must look like s3a://<bucket>/..."
}
$BUCKET = $Matches[1]

$SCRIPT_S3 = "s3://$BUCKET/jodi-oil/jobs/week4/spark_to_gold.py"
$CONFIG_S3 = "s3://$BUCKET/jodi-oil/jobs/week4/config.yaml"

& $aws1 s3 cp Week4/code/spark_to_gold.py $SCRIPT_S3 --profile $PROFILE --region $REGION
...
$sparkSubmitParameters = @(
  ...
  "--conf spark.emr-serverless.driverEnv.GOLD_URI=$($envMap['GOLD_URI'])",
...
$jobDriverPath = Join-Path $env:TEMP "jodi-week4-job-driver.json"
$configOverridesPath = Join-Path $env:TEMP "jodi-week4-config-overrides.json"

$jobDriver | ConvertTo-Json -Depth 20 | Set-Content -Path $jobDriverPath -Encoding ascii
$configOverrides | ConvertTo-Json -Depth 20 | Set-Content -Path $configOverridesPath -Encoding ascii
...
if ($state -ne 'SUCCESS') {
  throw "Gold job finished with state: $state"
}
```

## 4) Verify Gold output
```powershell
$GOLD_URI_S3 = $envMap['GOLD_URI'] -replace '^s3a://', 's3://'
& $aws1 s3 ls $GOLD_URI_S3 --recursive --profile $PROFILE --region $REGION
```
